...
void UART_Init(uint8_t v_uartChannel_u8, uint32_t v_baudRate_u32)
{
    ...
    else if(v_uartChannel_u8 == C_UartOne_U8)
    {
        ...
        UCSR1C= (1<<UCSZ11) | (1<<UCSZ10);                // Asynchronous mode 8-bit data and 1-stop bit
...
char UART_RxChar(uint8_t var_uartChannel_u8)
{
	...
	switch(var_uartChannel_u8)
	{
		...
		case 1:
		while(util_IsBitCleared(UCSR1A,RXC1));  // Wait till the data is received
		ch = UDR1;
		break;
...
void UART_TxChar(uint8_t v_uartChannel_u8, char v_uartData_u8)
{
    ...
    {
        while(util_IsBitCleared(UCSR1A,UDRE1)); // Wait till Transmitter(UDR) register becomes Empty
        UDR1 =v_uartData_u8; 
    }
}
